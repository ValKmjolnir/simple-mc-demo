...
void light(){
    ...
    static GLfloat ambientlight[]={0.8,0.8,0.8,1};
    static float x=1;
    ...
    for(int i=0;i<3;++i){
        ambientlight[i]+=x*0.001;
        diffuselight[i]+=x*0.001;
    }
    if(ambientlight[0]>=0.95)
        x=-1;
    else if(ambientlight[0]<=0.15)
        x=1;
...
void Draw3d(void)
{
    ...
    for(int i=0;i<65;++i)
        for(int j=0;j<65;++j){
            world[i][j].setblock();
            double y=world[i][j].get_pos().y-1;
            for(int k=0;k<2;++k){
                setblock_pos(i-32.0,y-k,j-32.0);
            }
        }
}
